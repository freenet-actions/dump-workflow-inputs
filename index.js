...
(async () => {
  try {
    ...
    core.info(core.getBooleanInput('write-summary'));
    if (core.getBooleanInput('write-summary')) {
      core.info('Writing summary');
      const factsRows = [[{data: 'Input', header: true}, {data: 'Value', header: true}]];
      for(const fact of facts) {
        factsRows.push([fact.label, fact.value]);
      }
